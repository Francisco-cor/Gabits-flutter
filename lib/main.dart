...
// import 'package:flutter_localizations/flutter_localizations.dart'; // Puedes probar a quitar esta si no hay errores
...
class MyApp extends StatelessWidget {
  ...
  static const Color accentColor = Color(0xFF4FD1C5); // Color de acento original
  ...
  static const String _preferredFontFamily = '.SF UI Text';

  // Nuevos colores para el FAB
  static const Color fabBackgroundColor = Color(0xFF1A202C); // Negro (o darkText)
  static const Color fabForegroundColor = Colors.white;     // Blanco
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        colorScheme: ColorScheme(
          ...
          secondary: accentColor, // Puedes mantener tu color de acento original aquí
          ...
          onSecondaryContainer: accentColor, // Considera si esto debe cambiar
        ...
        appBarTheme: AppBarTheme(
          ...
          iconTheme: const IconThemeData(color: primaryAppColor, size: 24.0),
          actionsIconTheme: const IconThemeData(color: primaryAppColor, size: 24.0),
        ...
        floatingActionButtonTheme: FloatingActionButtonThemeData(
            backgroundColor: fabBackgroundColor, // Color de fondo del FAB
            foregroundColor: fabForegroundColor, // Color del texto e icono del FAB
            elevation: 4.0,
            hoverElevation: 6.0,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16.0)),
            extendedTextStyle: const TextStyle(
                ...
                fontSize: 16,
                color: fabForegroundColor // Asegura que el texto también use este color
            )
        ),
      ),
...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
  ...
  String _getRemainingTime(TimeOfDay habitStartTime, AppLocalizations localizations) {
    final now = DateTime.now();
    final habitDateTimeToday = DateTime(now.year, now.month, now.day, habitStartTime.hour, habitStartTime.minute);
    final difference = habitDateTimeToday.difference(now);
    if (difference.isNegative) {
      final absDifference = difference.abs();
      if (absDifference.inDays > 0) return localizations.passedDaysAgo(absDifference.inDays);
      if (absDifference.inHours > 0) return localizations.passedHoursAgo(absDifference.inHours);
      return localizations.passedStatus;
    }
    if (difference.inHours > 0) return '${difference.inHours}h ${difference.inMinutes.remainder(60)}m';
    if (difference.inMinutes > 0) return '${difference.inMinutes}m ${difference.inSeconds.remainder(60)}s';
  ...
  DiaryEntry? _getDiaryEntryForDate(DateTime date) {
    ...
    try {
      return _allDiaryEntries.firstWhere((e) => isSameDay(e.date, normalizedDate));
    } catch (e) {
      return null;
    }
  }
        ...
        return Stack(
          children: <Widget>[
            ...
            CompositedTransformFollower(
              ...
              child: _buildFabMenuContent(context, localizations, Theme.of(context)),
            ),
          ],
        );
      },
  ...
  Widget _buildFabMenuContent(BuildContext buildContextForNavigation, AppLocalizations localizations, ThemeData theme) {
    final menuOptions = [
      {'icon': Icons.edit_calendar_outlined, 'label': localizations.newDiaryEntryOption, 'action': () {
        _closeFabMenu();
        ...
        )).then((_) { if (mounted) _loadInitialDiaryEntries(); });
      }},
      {'icon': Icons.sticky_note_2_outlined, 'label': localizations.newNoteOption, 'action': () async {
        _closeFabMenu();
        final dynamic returnedValue = await Navigator.of(buildContextForNavigation).push(
        ...
        if (returnedValue is Note) {
          ...
          Navigator.of(buildContextForNavigation).push(
            MaterialPageRoute(builder: (notesScreenContext) => NotesScreen(newlyAddedNote: newNote)),
          );
        }
      }},
      {'icon': Icons.add_task_rounded, 'label': localizations.newHabitOption, 'action': () async {
        _closeFabMenu();
        final Habit? nH = await Navigator.of(buildContextForNavigation).push<Habit>(
    ...
    return Material(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: menuOptions.map((o) => _buildFabMenuButton(
        ...
        ).animate().fadeIn(duration: 120.ms).scaleXY(begin: 0.9, end: 1.0, curve: Curves.decelerate),
      ),
    );
  }

  Widget _buildFabMenuButton({ required BuildContext context, required IconData icon, required String label, required FutureOr<void> Function() onPressed, required ThemeData theme}) {
    return InkWell(
      onTap: onPressed,
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Text(label, style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w500, color: theme.colorScheme.onSurface, fontSize: 15)),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final Color greetingIconColor = theme.iconTheme.color?.withOpacity(0.8) ?? MyApp.primaryAppColor.withOpacity(0.8);
    final Color noHabitsIconColor = theme.colorScheme.secondary.withOpacity(0.6);
    final Color noHabitsTextColor = theme.colorScheme.onBackground.withOpacity(0.7);
    final Color cardStripeColorCompleted = Colors.grey.shade400;
    final Color cardTextColorCompleted = theme.colorScheme.onSurface.withOpacity(0.55);
    final Color remainingTimePassedColor = Colors.red.shade400.withOpacity(0.8);
    final Color remainingTimeStartsNowColor = theme.colorScheme.secondary.withOpacity(0.9);
    final Color checkIconCompletedColor = Colors.green.shade500;
    final Color checkIconNotCompletedColor = theme.colorScheme.secondary.withOpacity(0.85);
    final Color topButtonSplashColor = theme.colorScheme.secondary.withOpacity(0.1);
    final Color topButtonHighlightColor = theme.colorScheme.primary.withOpacity(0.05);
    final Color topButtonTextColor = theme.colorScheme.onBackground.withOpacity(0.9);
      ...
      appBar: AppBar(
        title: Row(
          children: [
            ...
            Text(_getGreeting(context), style: theme.appBarTheme.titleTextStyle),
          ],
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => CalendarScreen(allHabits: _allHabits))),
            ...
            child: Row(
              ...
              children: <Widget>[
                _buildTopButton(
                  ...
                  onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => HabitsScreen(
                    allHabits: _allHabits,
                    onUpdateHabit: _updateHabit,
                    onDeleteHabit: _deleteHabit,
                    onAddHabit: _addHabit,
                  ))).then((_) { if (mounted) { _loadInitialDataFromIsar(); }}),
                  splashColor: topButtonSplashColor,
                  highlightColor: topButtonHighlightColor,
                  textColor: topButtonTextColor,
                ).animate().fadeIn(delay: 400.ms, duration: 500.ms).slideY(begin: 0.3, curve: Curves.easeOutCubic),
                _buildTopButton(
                  context: context,
                  ...
                  highlightColor: topButtonHighlightColor,
                  textColor: topButtonTextColor,
                ).animate().fadeIn(delay: 500.ms, duration: 500.ms).slideY(begin: 0.3, curve: Curves.easeOutCubic),
                _buildTopButton(
                  context: context,
                  ...
                  highlightColor: topButtonHighlightColor,
                  textColor: topButtonTextColor,
                ).animate().fadeIn(delay: 600.ms, duration: 500.ms).slideY(begin: 0.3, curve: Curves.easeOutCubic),
              ],
            ),
          ),
          Padding(
            ...
            child: _dailyRoutineHabits.isEmpty
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Icon(Icons.space_dashboard_outlined, size: 72, color: noHabitsIconColor),
                    const SizedBox(height: 24),
                    Text(
                      localizations.noHabitsToday,
                      style: TextStyle(fontSize: 19, fontWeight: FontWeight.w500, color: noHabitsTextColor),
                ...
                : ListView.builder(
              padding: const EdgeInsets.symmetric(horizontal: 16.0).copyWith(bottom: 90.0, top: 4.0),
              ...
              itemBuilder: (context, index) {
                ...
                final now = DateTime.now();
                final habitDateTimeToday = DateTime(now.year, now.month, now.day, habit.startTime.hour, habit.startTime.minute);
                ...
                final String remainingTimeText = _getRemainingTime(habit.startTime, localizations);
                bool startsNow = remainingTimeText == localizations.startsNowStatus && !habit.isCompleted;
                ...
                Color detailIconsColor = theme.iconTheme.color?.withOpacity(0.7) ?? theme.colorScheme.onSurfaceVariant;
                ...
                if (habit.isCompleted) {
                  ...
                  detailIconsColor = currentCardTextColor.withOpacity(0.8);
                } else {
                  currentCardStripeColor = habit.color;
                  if (isPassed) {
                    detailIconsColor = theme.colorScheme.onSurface.withOpacity(0.75);
                  }
                }
                ...
                if (cardShape is RoundedRectangleBorder) {
                  final resolvedShape = cardShape.borderRadius.resolve(Directionality.of(context));
                  ...
                  child: IntrinsicHeight(
                    child: Row(
                      ...
                      children: [
                              ...
                              children: [
                                Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    decorationColor: currentCardTextColor.withOpacity(0.8),
                                  ),
                                  maxLines: 1,
                                ...
                                Row(
                                  children: [
                                    Icon(Icons.schedule_rounded, size: 17, color: detailIconsColor),
                                    ...
                                    Text(
                                      ...
                                      style: TextStyle(
                                        ...
                                        color: currentCardTextColor.withOpacity(0.9),
                                        decoration: cardTextDecoration,
                                        decorationColor: currentCardTextColor.withOpacity(0.8),
                                      ),
                                    ),
                                ...
                                if (habit.goalType != GoalType.yesNo && habit.goalValue != null && habit.goalValue! > 0)
                                  Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        Icon(
                                            habit.goalType == GoalType.time ? Icons.timer_outlined : Icons.format_list_numbered_rounded,
                                            size: 16,
                                        ...
                                        Text(
                                          ...
                                          style: TextStyle(
                                            ...
                                            color: currentCardTextColor.withOpacity(0.8),
                                            decoration: cardTextDecoration,
                                            decorationColor: currentCardTextColor.withOpacity(0.8),
                                          ),
                                        ),
                        ...
                        if (habit.goalType == GoalType.yesNo && (!isPassed || habit.isCompleted) )
                          Padding(
                            padding: const EdgeInsets.only(right: 8.0),
                            child: Tooltip(
                              message: habit.isCompleted ? localizations.done : localizations.markAsDone,
                              child: IconButton(
                                icon: Icon(
                                  habit.isCompleted ? Icons.check_circle_rounded : Icons.check_circle_outline_rounded,
                                  color: habit.isCompleted ? checkIconCompletedColor : checkIconNotCompletedColor,
                                  size: 28,
                                ),
                                onPressed: () async {
                                  final String habitNameForSnackbar = habit.name;
                                  ...
                                  if (mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                        SnackBar(
                                          content: Text('$habitNameForSnackbar ${!wasCompleted ? localizations.markedAsDone : localizations.markedAsNotDone}'),
                                          backgroundColor: !wasCompleted ? Colors.green.shade600 : theme.colorScheme.primary,
                ...
                ).animate().fadeIn(duration: 350.ms, delay: (index * 70).ms).slideX(begin: 0.05, curve: Curves.easeOutCubic);
              },
            ),
          ),
        ],
      ...
      floatingActionButton: CompositedTransformTarget(
        ...
        child: FloatingActionButton.extended(
          ...
          icon: AnimatedIcon(
            ...
            // color: Colors.black, // REMOVIDO para que tome el color del theme
          ),
        ).animate()
            .slideY(begin: 2, end: 0, duration: 500.ms, delay: 800.ms, curve: Curves.elasticOut)
            .fadeIn(delay: 800.ms),
      ),
